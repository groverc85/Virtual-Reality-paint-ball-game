...
public class GyroController : MonoBehaviour
{
    ...
    void Update()
    {
        if (controlledObject != null && !Paused)
        {
            ...
            //   data from gyroscope
//			controlledObject.transform.Rotate(Input.gyro.rotationRateUnbiased.x, Input.gyro.rotationRateUnbiased.y, 
//				Input.gyro.rotationRateUnbiased.z);

			Quaternion qRotation = Quaternion.Inverse(qRefGyro) * ConvertRotation(gyro.attitude);
			controlledObject.transform.rotation = qRefObject * qRotation;
        }
    }
